...
awk -v var=$EV '{if (NR%var==0) print $0}' all/coord.raw > coord.raw
awk -v var=$EV '{if (NR%var==0) print $0}' all/force.raw > force.raw
awk -v var=$EV '{if (NR%var==0) print $0}' all/energy.raw > energy.raw
awk -v var=$EV '{if (NR%var==0) print $0}' all/box.raw > box.raw
